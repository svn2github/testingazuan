...
public class BirtReportServlet extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
        ...
        logger.debug(this.getClass().getName() +":init:Initializing SpagoBI BirtReport Engine...");
        ...
        logger.debug(this.getClass().getName() +":init:Inizialization " +
	      "of SpagoBI BirtReport Engine ended succesfully");
	...
	protected void runReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//test --------------------
		/*
		SpagoBiDataSource ds = findDataSource(documentId);
		if (ds != null){
			logger.debug("DataSource founded.");
			if(ds.getJndiName() != null && ds.getJndiName() != ""){
				reportParams.put("connectionName", ds.getJndiName());
			}
			else{
				BirtJDBCDataSource jdbc = new BirtJDBCDataSource(ds.getDriver(), ds.getUrl(), ds.getUser(), ds.getPassword());
				DataEngine de = DataEngine.newDataEngine( DataEngineContext.newInstance( DataEngineContext.DIRECT_PRESENTATION, null,null,null ) );
				de.defineDataSource(jdbc.getOdaDataSourceDesign());
			}
		}
		*/
		//fine test --------------------
