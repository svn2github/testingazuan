...
public class SetBCDetails_page01 extends WizardPage {
	...
	public SetBCDetails_page01(DBConnection conn, String table) {
	...
	public void createControl(Composite parent) {
		 ...
		 detailGroup.setLayoutData(gd);
		 		...
		 		name.setText(table.substring(0,1).toUpperCase() + table.substring(1));
		 		//------------------------------------------------------
		 //------------------------------------------------------------
		 Group keysGroup = new Group(container, SWT.SHADOW_ETCHED_IN);
		 keysGroup.setText("PK setting");
	...
	private void addItem() {
		if (conn.Start()){
			    ...
			    List<ColumnInfo> columns = conn.getColumns(table);
			    ...
			    while(itr.hasNext()){
					...
					if (ci.isKey()){
						TableItem ti = new TableItem(keys, 0);
						ti.setText(ci.getColumnName()+" - "+ci.getColumnType());
					}
					else{
						TableItem ti = new TableItem(attributes, 0);
						ti.setText(ci.getColumnName()+" - "+ci.getColumnType());
