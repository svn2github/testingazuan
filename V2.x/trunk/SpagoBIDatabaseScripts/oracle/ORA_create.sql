...
ALTER TABLE SBI_DIST_LIST_USER ADD  FOREIGN KEY (LIST_ID) REFERENCES SBI_DISTRIBUTION_LIST (DL_ID)  ON DELETE CASCADE
/

ALTER TABLE SBI_DIST_LIST_OBJECTS ADD  FOREIGN KEY (DL_ID) REFERENCES SBI_DISTRIBUTION_LIST (DL_ID)  ON DELETE CASCADE
/
...
NOORDER
;
CREATE SEQUENCE SBI_DOSS_BINARY_CONT_TEMP_SEQ 
INCREMENT BY 1 
START WITH 1 
NOMAXVALUE 
NOMINVALUE 
...
CREATE TRIGGER TRG_SBI_DISTRIBUTION_LIST
  BEFORE INSERT
  ON SBI_DISTRIBUTION_LIST
  REFERENCING OLD AS OLD NEW AS NEW
  FOR EACH ROW
  DECLARE NUOVO_ID NUMBER;
BEGIN
IF :NEW.DL_ID IS NULL THEN
     SELECT SBI_DISTRIBUTION_LIST_SEQ.NEXTVAL INTO NUOVO_ID FROM DUAL;
     :NEW.DL_ID:=NUOVO_ID;
END IF;
END;
/
...
CREATE TRIGGER TRG_SBI_DOSS_BINARY_CONT_TEMP
  BEFORE INSERT
...
IF :NEW.BIN_ID IS NULL THEN
     SELECT SBI_DOSS_BINARY_CONT_TEMP_SEQ.NEXTVAL INTO NUOVO_ID FROM DUAL;
     :NEW.BIN_ID:=NUOVO_ID;
END IF;
END;
/
