...
CREATE INDEX label_unique ON SBI_ALARM(LABEL);

CREATE INDEX name_unique ON SBI_ALARM_CONTACT(NAME);

CREATE INDEX code_unique ON SBI_KPI(code); 

CREATE INDEX unique_kpi_model_cd ON SBI_KPI_MODEL(KPI_MODEL_CD); 

CREATE INDEX name_unique ON SBI_KPI_PERIODICITY(name); 

ALTER TABLE SBI_THRESHOLD MODIFY COLUMN code VARCHAR(45) NOT NULL;
CREATE INDEX code_unique ON SBI_THRESHOLD(code); 

ALTER TABLE SBI_THRESHOLD_VALUE MODIFY COLUMN label VARCHAR(20) NOT NULL;
CREATE INDEX label_thId_unique ON SBI_THRESHOLD_VALUE(label, THRESHOLD_ID); 

CREATE INDEX unique_resource_name ON SBI_RESOURCES(RESOURCE_NAME); 
