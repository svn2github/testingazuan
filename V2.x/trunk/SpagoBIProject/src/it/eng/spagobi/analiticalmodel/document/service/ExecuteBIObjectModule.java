...
public class ExecuteBIObjectModule extends AbstractModule {
	...
	if (idStr == null && label == null) {
	    if (obj == null) {
		logger.error("The object id and label are not set and no objects are in session");
	...
	if ((controller.directExecution() && ((subObjects.size() == 0 && snapshots.size() == 0 && viewpoints.size() == 0) || ignoreSubNodes))
		|| subObj != null) {
	    logger.debug("object hasn't any parameter to fill and no subObjects");
	    if (subObj == null)
	...
	if (controller.directExecution()) {
	    logger.debug("object has only subobjects but not parameter to fill");
	...
	if (parameterNameFromLookUp == null)
	    parameterNameFromLookUp = (String) session.getAttribute("LOOKUP_PARAMETER_NAME");
     ...
     *                The BIobject
     * @param subObj
     *                The SubObjectDetail subObject to be executed (in case it
     *                is not null)
     * @param response
     *                The response Source Bean
     */
	...
	if (!compatibleBiobjTypeCd.equalsIgnoreCase(biobjTypeCd)) {
	    ...
	    logger.warn("Engine cannot execute input document type: " + "the engine " + engine.getName()
		    + " can execute '" + compatibleBiobjTypeCd + "' type documents "
	...
	if ("EXT".equalsIgnoreCase(engineType.getValueCd())) {
		...
		logger.error("The class ['" + className + "'] for internal engine " + engine.getName()
			+ " was not found.", cnfe);
		Vector params = new Vector();
	...
	if (isLookupCall(request)) {
		...
		logger.error("The BIParameter with id = " + getLookedUpParameterId(request).toString()
			+ " does not exist.");
	    ...
	    String correlatedParuseId = (String) request.getAttribute("correlatedParuseIdForObjParWithId_"
		    + lookupObjParId);
	...
	while (iterParams.hasNext()) {
		...
		for (int i = 0; i < values.size(); i++) {
			...
			logger.warn("Parameter '" + biparam.getLabel() + "' cannot assume value '" + value + "'"
				+ " for user '" + profile.getUserUniqueIdentifier().toString() + "' with role '" + role
				+ "'.");
			List l = new ArrayList();
	...
	try {
		...
		if (vp.getVpScope().equals(
			PortletUtilities.getMessage("SBIDev.docConf.viewPoint.scopePrivate", "messages"))
