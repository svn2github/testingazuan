...
public class CopyofDocumentCompositionUtils {
	...
	public static String getUrl(String objLabel, SessionContainer sessionContainer, SourceBean requestSB) {
		logger.debug("IN");
		...
		//get engine type
		try{
			doc = (BIObject)DAOFactory.getBIObjectDAO().loadBIObjectByLabel(objLabel);
			if (doc == null){
		    ...
		    urlReturn = engine.getUrl()+"?userId="+userProfile.getUserUniqueIdentifier()+"&"+SpagoBIConstants.SBICONTEXTURL+"="+GeneralUtilities.getSpagoBiContextAddress();
		}
		catch (Exception ex) {		
		...
		urlReturn += getParametersUrl(doc, docConfig, requestSB);
	...
	public static String getUrl(String objLabel, DocumentCompositionConfiguration docConfig, SourceBean requestSB) {
		...
		try{
		    ...
		    urlReturn = engine.getUrl();
		}
		catch (Exception ex) {		
			logger.error("Cannot obtain engine url " , ex);		
		}	
	//	urlReturn += "?document="+doc.getId();
		urlReturn += getParametersUrl(doc, docConfig, requestSB);
		logger.debug("OUT");
		return urlReturn;
	}
	...
	public static EngineURL getEngineUrl(String objLabel, DocumentCompositionConfiguration docConfig, SourceBean requestSB) {
		...
		try{
		    ...
		    if (engineurl!=null && engineurl.lastIndexOf("/")>0){
				...
				urlReturn = engineurl.subSequence(0, l).toString();	
				logger.debug("Engine url " + urlReturn);		
		    }
		    
		    HashMap parameters = new HashMap();
			parameters.put("document", doc.getId());
		    //parameters.put("forward", "editQuery.jsp");
			//applySecurity(parameters, profile);
			//addLocale(parameters);
			engineURL = new EngineURL(urlReturn, parameters);
		}
		catch (Exception ex) {		
			logger.error("Cannot obtain engine url " , ex);		
		}	
	...
	private static String getParametersUrl(BIObject doc, DocumentCompositionConfiguration docConfig, SourceBean requestSB){
		String paramUrl = "";
		...
		for (int i = 0; i < lstDoc.size(); i++){
			...
			if (document != null){
				if (!doc.getLabel().equalsIgnoreCase(document.getSbiObjLabel()))
					continue;
				else{
					foundDoc = true;
					break;
	...
	private static String getParametersUrlAsEnum(BIObject doc, DocumentCompositionConfiguration docConfig, SourceBean requestSB){
		return "";
	}
	...
	public static String getEngineUrl(String objLabel, SessionContainer sessionContainer, SourceBean requestSB) {
		...
		if ("EXT".equalsIgnoreCase(engineType.getValueCd())) {
			try {
				...
				while(parKeysIter.hasNext()) {
		     	...
		     	urlReturn += getParametersUrl(obj, docConfig, requestSB);
		...
		//adapt url for particular case 
		//if into url therisn't document id, it will be added
		if (urlReturn.indexOf("document=") < 0)
			urlReturn += "&document=" + obj.getId();
		/*
		//checks if into url there is a reset of the session. In positive case it will be deleted 
		String tmpStr = "NEW_SESSION=TRUE";
		int tmpInit = urlReturn.indexOf(tmpStr);
		if ( tmpInit >= 0){
			String tmpChar = urlReturn.substring(tmpInit-1,tmpInit);
			if (tmpChar.equalsIgnoreCase("&"))
				tmpInit = tmpInit-1;
			String tmpStrOut = urlReturn.substring(tmpInit, tmpInit + tmpStr.length()+1);
			urlReturn = urlReturn.replace(tmpStrOut, "");
		}
		*/
		return urlReturn;
	}
