...
public class TransformerFrom2_4_0To2_5_0 implements ITransformer {
	...
	private void fixKpiModel(Connection conn) throws Exception {
		...
		String sql =  "ALTER TABLE SBI_KPI_MODEL ADD COLUMN KPI_MODEL_LBL VARCHAR(1000)";
		stmt.execute(sql);
	...
	private void fixModelInstance(Connection conn) throws Exception {
		...
		String sql =  "ALTER TABLE SBI_KPI_MODEL_INST ADD COLUMN MODELUUID VARCHAR(400) DEFAULT NULL;";
		stmt.execute(sql);
	...
	private void fixThresholdValue(Connection conn) throws Exception {
		...
		String sql =  "ALTER TABLE SBI_THRESHOLD_VALUE ADD COLUMN MIN_CLOSED BOOLEAN DEFAULT FALSE";
		stmt.execute(sql);
