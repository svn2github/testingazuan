...
<%!
String getExecutionUrl(String baseUrl, Map mapPars) {
...
executionParameters.putAll(documentParametersMap);
...
<div class="header">
	...
	<div class="toolbar_header">
		<ul>
		    ...
		    <li>		    
				<a style="text-decoration:none;" href='<%= getExecutionUrl(execUrl, documentParametersMap)  %>'>
	...
	if(!win_sendTo_<%= uuid %>) {
		win_sendTo_<%= uuid %> = new Ext.Window({
			...
			bodyCfg: {
				...
				children:[{
					tag:'iframe',
      					src: '<%= getExecutionUrl(request.getContextPath() + GeneralUtilities.getSpagoAdapterHttpUrl() + "?ACTION_NAME=SHOW_SEND_TO_FORM&objlabel=" + obj.getLabel(), documentParametersMap) %>',
      					frameBorder:0,
      					width:'100%',
      					height:'100%',
      					style: {overflow:'auto'}  
...
<div id="divIframe<%= uuid %>" style="width:100%;overflow=auto;border: 0;display:inline;"> <!-- float:left; -->
	<iframe id="iframeexec<%= uuid %>" name="iframeexec<%= uuid %>" src="<%= getExecutionUrl(obj.getEngine().getUrl(), executionParameters) %>" style="width:100%;height:300px;z-index:0;" frameborder="0" >
	</iframe>
</div>
