 ...
 * @author sulis
 */
public class ListObjectsHtmlGeneratorDevImpl implements IListObjectsHtmlGenerator {
...
//				htmlStream.append(" </td>\n");
	...
	private void createDetailObjectLink(String path, StringBuffer htmlStream) {
		...
		String capTitle = PortletUtilities.getMessage("SBISet.devObjects.captionDetail", "messages");
	...
	private void createEraseObjectLink(String path, StringBuffer htmlStream) {
		...
		addUrl.setParameter(ObjectsTreeConstants.PATH, path);
		addUrl.setParameter(SpagoBIConstants.ACTOR, SpagoBIConstants.DEV_ACTOR);
		String capTitle = PortletUtilities.getMessage("SBISet.devObjects.captionErase", "messages");
	...
	/**
	 * Creates the button in order to access the biparameters list of the object .
	 * It adds the html code to the <code>htmlString</code> received as input. 
	 * 
	 * @param path The String representing the object's path
	 * @param htmlStream The String Buffer  to whom code is added
	 */
	private void createListBiParametersLink(String path, String id, StringBuffer htmlStream) {
		...
		listUrl.setParameter(ObjectsTreeConstants.OBJECT_ID, id);
		listUrl.setParameter(ObjectsTreeConstants.PATH, path);
		String capTitle = PortletUtilities.getMessage("SBISet.devObjects.captionListPar", "messages");
        htmlStream.append("<a title=\""+capTitle+"\" class=\"linkOperation\" href='"+listUrl.toString()+"'>\n");
        htmlStream.append("<img src='"+renderResponse.encodeURL(renderRequest.getContextPath() + "/img/listBiParameters.gif" )+"' /></a>\n");
	}
	...
	protected void makeNavigationButton(ListIFace list) {
		...
		nextUrl.setParameter(SpagoBIConstants.ACTOR, SpagoBIConstants.DEV_ACTOR);
		...
		prevUrl.setParameter(SpagoBIConstants.ACTOR, SpagoBIConstants.DEV_ACTOR);
		...
		allUrl.setParameter(SpagoBIConstants.ACTOR, SpagoBIConstants.DEV_ACTOR);
		...
		filterURL.setParameter(SpagoBIConstants.ACTOR, SpagoBIConstants.DEV_ACTOR);
...
//		String formId = "formFilter" + formSuffix;
	...
	private ListIFace filterList(ListIFace listAll) {
		...
		for(int i=0; i<rows.size(); i++) {
	        ...
	        String path = (String)row.getAttribute("Path"); 	
	        String state = (String)row.getAttribute("State"); 
	        if(ObjectsAccessVerifier.canDev(state, path, profile) || 
