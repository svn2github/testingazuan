...
/**
 * Publishes the results of a detail request for a BI objects parameters into the correct 
 * jsp page according to what contained into request. If Any errors occurred during the 
 * execution of the <code>DetailBIObjectParametersModule</code> class, the publisher
...
public class DetailBIObjectParameterPublisher implements PublisherDispatcherIFace {
	...
	public String getPublisherName(RequestContainer requestContainer, ResponseContainer responseContainer) {
		...
		SourceBean moduleResponse = (SourceBean)responseContainer.getServiceResponse().getAttribute("DetailBIObjectParameterModule");
		
		// if the module response is null throws an error and return the name of the errors publisher
		if(moduleResponse==null) {
			SpagoBITracer.major(SpagoBIConstants.NAME_MODULE, 
		            "DetailBIObjectParameterPublisher", 
		            "getPublisherName", 
		...
		Object loop = null;
		try{
			loop = moduleResponse.getAttribute("loopback");
		} catch (Exception e) {
			/* ignore */
		}
        
		if(loop != null) {
        	return new String("DetailBIObjectParameterLoop");
		} else {
			return new String("DetailBIObjectParameter");
		}
