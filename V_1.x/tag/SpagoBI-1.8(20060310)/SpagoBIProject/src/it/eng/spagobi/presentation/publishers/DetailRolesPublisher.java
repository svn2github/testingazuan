 ...
 * @author sulis
 */
public class DetailRolesPublisher implements PublisherDispatcherIFace {
	...
	public String getPublisherName(RequestContainer requestContainer, ResponseContainer responseContainer) {
		...
		SourceBean moduleResponse = (SourceBean)responseContainer.getServiceResponse().getAttribute("DetailRolesModule");
		
		// if the module response is null throws an error and return the name of the errors publisher
		if(moduleResponse==null) {
			SpagoBITracer.major(SpagoBIConstants.NAME_MODULE, 
		            "DetailRolesPublisher", 
		            "getPublisherName", 
		...
		if(!errorHandler.isOKBySeverity(EMFErrorSeverity.ERROR)) {
			String responseComplete = (String) moduleResponse.getAttribute(SpagoBIConstants.RESPONSE_COMPLETE);
			if ("true".equalsIgnoreCase(responseComplete)) return "detailRoles";
			else return "error";
		}

        Object loop = moduleResponse.getAttribute("loopback");
        if (loop != null) {
        	return "detailRolesLoop";
		} else {
			return "detailRoles";
		}
