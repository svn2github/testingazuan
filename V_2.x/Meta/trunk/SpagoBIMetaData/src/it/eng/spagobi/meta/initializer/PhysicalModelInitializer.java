...
public class PhysicalModelInitializer {
	...
	private void initCatalogMeta(PhysicalModel model, Connection conn, String defaultCatalog) {
		try {
			model.setCatalog( retriveCatalog(conn, defaultCatalog) );
		} catch(Throwable t) {
	...
	private void initSchemaMeta(PhysicalModel model, DatabaseMetaData dbMeta, String defaultSchema) {
		try {
			model.setSchema( retriveSchema(dbMeta, defaultSchema) );
		} catch(Throwable t) {
	...
	/**
	 * 
	 * @deprecated test 4 future use
	 */
	private void initTypesMeta(PhysicalModel model, DatabaseMetaData dbMeta) {
		Map typeMap;
		ResultSet rs;
		...
		try {
			 ...
			 */
			while (rs.next()) {
				typeMap.put( rs.getInt("DATA_TYPE"), rs.getString("TYPE_NAME") );
				log(rs.getInt("DATA_TYPE") + " -> "+  rs.getString("TYPE_NAME"));
			}
		
		} catch(Throwable t) {
			throw new RuntimeException("Impossible to initialize types metadata", t);
		}
	}
	...
	private void initTablesMeta(PhysicalModel model, DatabaseMetaData dbMeta) {
		try {
			model.addTables( retriveTables(dbMeta, model.getCatalog(), model.getSchema()) );
		} catch(Throwable t) {
	...
	private void initForeignKeysMeta(PhysicalModel model, DatabaseMetaData dbMeta) {
		try {
			for(int i = 0; i < model.getTables().size(); i++) {
				model.addForeignKeys( retriveForeignKey(dbMeta, model.getCatalog(), model.getSchema(), model.getTables().get(i).getName()) );
			}
	...
	private void initPrimaryKeysMeta(PhysicalModel model, DatabaseMetaData dbMeta) {
		...
		try {
			for(int i = 0; i < model.getTables().size(); i++) {
				primaryKey = retrivePrimaryKey(dbMeta, model.getCatalog(), model.getSchema(), model.getTables().get(i).getName());
				if(primaryKey != null) {
	...
	public PhysicalModel initizlize(String modelName, Connection conn, String defaultCatalog, String defaultSchema) {
		...
		//log("MODEL name: " + model.getName()); 
		//log("Catalog: " + model.getCatalog()); 
		//log("Schema: " + model.getSchema()); 
