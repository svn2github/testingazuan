 ...
 *
 */
public class RemoveBusinessColumnAction extends BusinessModelAction {

	public RemoveBusinessColumnAction(BusinessModelView businessModelView) {
		super(businessModelView);
		setText("Remove Business Column");
		setToolTipText("Remove Business Column");
		setImageDescriptor(Activator.getImageDescriptor("remove.png"));
	}

	public void run() {
		BusinessColumn businessColumn = (BusinessColumn)getTargetObject();
		businessColumn.getTable().getColumns().remove(businessColumn);
		MessageDialog.openInformation(
				null, 
				"Business Model Editor", 
				"Business Column Removed"
		);
	}
}
