...
public class GlobalConnectionPool implements IConnectionPool {
	...
	private Map pools = new HashMap(); //key: host, value: ServerConnectionPool;
	...
	public Connection getConnection(String serverId) {
		...
		synchronized (pools) {
			...
			if(pool == null){
				...
				pool = new ServerConnectionPool(factory, server.getHost(), server.getService(), server.getLogin(), 
						server.getPassword(), getMaxPoolConnections(), sProvider);
