...
import it.eng.spagobi.chiron.serializer.OrganizationalUnitJSONSerializer;
...
//				}
	...
	private void getOUChildrenNodes(Integer hierarchyId, Integer nodeId, Integer grantId){
		...
		if ( grantId == null ) {
			...
			if (nodeId == -1) {
				...
				ous = DAOFactory.getOrganizationalUnitDAO().getRootNodes(hierarchyId);
			} else {
				ous = DAOFactory.getOrganizationalUnitDAO().getChildrenNodes(nodeId);
			}
			...
			if (nodeId == -1) {
				ousWithGrants = DAOFactory.getOrganizationalUnitDAO().getRootNodesWithGrants(hierarchyId, grantId);
			} else {
	...
	private void getHierarchyRootNode(Integer hierarchyId, Integer grantId, List<Integer> modelInstances){
		...
		OrganizationalUnitHierarchy hierarchy = DAOFactory.getOrganizationalUnitDAO().getHierarchy(hierarchyId);
...
//			writeBackToClient( new JSONSuccess( grantsJSON ) );
	...
	private void insertGrant(JSONObject grantJSON,JSONArray grantNodesJSON){
		try {
			...
			if(grant.getId()!=null){
				...
				grant = DAOFactory.getOrganizationalUnitDAO().loadGrantByLabel(grant.getLabel());
				//first time save all
				grantNodes = getAllNodesToInsert(grant);
			}
			grant.setIsAvailable(true);
			logger.debug("prepared nodes to save of size:"+grantNodes.size());
			DAOFactory.getOrganizationalUnitDAO().insertNodeGrants(grantNodes, grant.getId());
	...
	private List <OrganizationalUnitGrantNode> getAllNodesToInsert(OrganizationalUnitGrant grant) throws EMFUserError{		
		...
		List<OrganizationalUnitNode> ouNodes = DAOFactory.getOrganizationalUnitDAO().getOrganizationalUnitNodeList(hierarchyId);
...
//	}
	...
	private List<OrganizationalUnitGrantNode> buildGrantForChilds(OrganizationalUnitNode ouNode, ModelInstanceNode modelInstanceNode,  OrganizationalUnitGrant grant){
		...
		if(ouChildrenByParentIDMap.get(parentId) == null){
			childOus = DAOFactory.getOrganizationalUnitDAO().getChildrenNodes(ouNode.getNodeId());
