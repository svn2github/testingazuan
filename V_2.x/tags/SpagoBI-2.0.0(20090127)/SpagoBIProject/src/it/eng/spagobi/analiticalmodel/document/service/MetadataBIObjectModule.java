...
public class MetadataBIObjectModule extends AbstractHttpModule {
	...
	public static final String MODULE_PAGE = "MetadataBIObjectPage";
	...
	/* (non-Javadoc)
	 * @see it.eng.spagobi.commons.services.BaseProfileModule#service(it.eng.spago.base.SourceBean, it.eng.spago.base.SourceBean)
	 */
	...
	throws Exception {
		...
		try {
			...
			if (message.trim().equalsIgnoreCase(SpagoBIConstants.METADATA_SELECT)) {
	...
	/**
	 * Gets the metadata of a BIObject choosen by the user from the 
	 * BIObjects list. It reaches the key from the request and asks to the DB all detail
	 ...
	 */   
	private void getMetadata(SourceBean request, SourceBean response) throws EMFUserError {		
		try {		 
			...
			response.setAttribute("biObj", obj);
		} catch (Exception ex) {
			logger.error("Cannot fill response container" + ex.getLocalizedMessage());	
			HashMap params = new HashMap();
			params.put(AdmintoolsConstants.PAGE, MetadataBIObjectModule.MODULE_PAGE);
