alter table SBI_AUDIT  ADD CONSTRAINT FK_SBI_AUDIT_1 foreign key (ENGINE_REF) references SBI_ENGINES (ENGINE_ID);\p\g
alter table SBI_AUDIT  ADD CONSTRAINT FK_SBI_AUDIT_2 foreign key (DOC_REF) references SBI_OBJECTS (BIOBJ_ID);\p\g
alter table SBI_AUDIT  ADD CONSTRAINT FK_SBI_AUDIT_3 foreign key (SUBOBJ_REF) references SBI_SUBOBJECTS (SUBOBJ_ID);\p\g
...
alter table SBI_DIST_LIST_OBJECTS  ADD CONSTRAINT FK_SBI_DIST_LIST_OBJECTS_1 foreign key (DL_ID) references SBI_DIST_LIST (DL_ID);\p\g
alter table SBI_DIST_LIST_OBJECTS ADD CONSTRAINT FK_SBI_DIST_LIST_OBJECTS_2 foreign key (DOC_ID) references SBI_OBJECTS (BIOBJ_ID);\p\g
alter table SBI_DIST_LIST_USER ADD CONSTRAINT FK_SBI_DIST_LIST_USER_1 foreign key (LIST_ID) references SBI_DIST_LIST (DL_ID);\p\g
alter table SBI_DOMAINS ADD CONSTRAINT FK_SBI_DOMAINS_1 foreign key (PARENT_ID) references SBI_DOMAINS (VALUE_ID);\p\g
alter table SBI_DOSSIER_BIN_TEMP ADD CONSTRAINT FK_SBI_DOSSIER_BIN_TEMP_1 foreign key (PART_ID) references SBI_DOSSIER_TEMP (PART_ID);\p\g
...
alter table SBI_MENU_ROLE ADD CONSTRAINT FK_SBI_MENU_ROLE_1 foreign key (EXT_ROLE_ID) references SBI_EXT_ROLES (EXT_ROLE_ID);\p\g
alter table SBI_MENU_ROLE ADD CONSTRAINT FK_SBI_MENU_ROLE_2 foreign key (MENU_ID) references SBI_MENU (MENU_ID);\p\g
...
alter table SBI_REMEMBER_ME ADD CONSTRAINT FK_SBI_REMEMBER_ME_1 foreign key (BIOBJ_ID) references SBI_OBJECTS (BIOBJ_ID);\p\g
alter table SBI_REMEMBER_ME ADD CONSTRAINT FK_SBI_REMEMBER_ME_2 foreign key (SUBOBJ_ID) references SBI_SUBOBJECTS (SUBOBJ_ID);\p\g
...
alter table SBI_KPI_INSTANCE ADD CONSTRAINT FK_SBI_KPI_INSTANCE_5 foreign key (id_kpi_periodicity) references SBI_KPI_PERIODICITY (id_kpi_periodicity);\p\g
