 ...
 */
	...
	private IDataSource getDatasourceToOpen(Integer dataSourceId){
		...
		try {
			...
			LogMF.trace(logger, "Retrieving datasource with id [{0}]", dataSourceId);
			
			IDataSourceDAO	dsDao = DAOFactory.getDataSourceDAO();
			dataSource = dsDao.loadDataSourceByID(dataSourceId);
			Assert.assertNotNull(dataSource, "Datasource with id [" + dataSourceId + "] not found");
			
			
		} catch(Throwable t) {
			throw new SpagoBIServiceException(SERVICE_NAME, "Impossible to retrieve data source with Id "+dataSourceId, t);
		} finally {
			logger.debug("OUT");
		}
...
//	private Map<String, String> getDatasetParameterValuesMapFromString(String parameters) {
...
//		try {
//			
//			JSONObject parameterValues = ObjectUtils.toJSONObject(parameters); 
//			logger.debug("Dataset parameters is equal to [" + parameterValues + "]");
//
...
//				}		
//			} else {
//				LogMF.trace(logger, "Dataset Parameter [{0}] not valorized", parameters);
//			}
//					
//		} catch(Throwable t) {
//			logger.error("Impossible to parse dataset parameters [" + parameters+ "]", t);
//		}
//		
//		return parametersMap;
//	}
