...
public class TwitterCacheFactory {
	...
	private static final Logger logger = Logger.getLogger(TwitterCacheFactory.class);

	public final String MYSQL = "MySql";

	/**
	 * Setting parameters for a DB
	 * 
	 * @param type
	 *            : the selected DB
	 * @return twtter cache interface
	 */
	public ITwitterCache getCache(String type) {

		logger.debug("Method getCache(): Start");

		if (type.equalsIgnoreCase(MYSQL)) {

			logger.debug("Method getCache(): Setting cache for selected DB - MYSQL -");
