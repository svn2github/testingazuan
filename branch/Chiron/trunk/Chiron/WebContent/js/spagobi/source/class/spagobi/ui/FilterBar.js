	...
	members : {
		...
		createFilterBar : function(){
			...
			*/
			...
			//var space = new qx.legacy.ui.embed.HtmlEmbed("<br><br>");
			//this.add(space);
			
			var combo1_label = new qx.legacy.ui.basic.Label("  The value of the column   ");
		/*	with(combo1_label){
				setTextAlign("center");
			}*/
			this.add(combo1_label);
			
			var combo1 = new qx.legacy.ui.form.ComboBox();
			var items1 = ["Label","Name","Description"];
			for(var i=0; i< items1.length; i++) {
              var item = new qx.legacy.ui.form.ListItem(items1[i]);
              combo1.add(item);
            }
			this.add(combo1);
			
			var combo2_label = new qx.legacy.ui.basic.Label(" as a ");
		/*	with(combo2_label){
				setTextAlign("center");
			}*/
			this.add(combo2_label);
			
			var combo2 = new qx.legacy.ui.form.ComboBox();
			var items2 = ["Label","Name","Description"];
			for(var i=0; i< items2.length; i++) {
              var item = new qx.legacy.ui.form.ListItem(items2[i]);
              combo2.add(item);
            }
			this.add(combo2);
			
			var dummy_label = new qx.legacy.ui.basic.Label(" ");
			this.add(dummy_label);
			
			var combo3 = new qx.legacy.ui.form.ComboBox();
			var items3 = ["starts with","ends with","contains","=","<","<=",">",">="];
			for(var i=0; i< items3.length; i++) {
              var item = new qx.legacy.ui.form.ListItem(items3[i]);
              combo3.add(item);
            }
			this.add(combo3);													 
			
			var dummy_label1 = new qx.legacy.ui.basic.Label(" ");
			this.add(dummy_label1);
			
			var txt1 = new qx.legacy.ui.form.TextField();
			with(txt1){
				width : 70
			}
			this.add(txt1);
