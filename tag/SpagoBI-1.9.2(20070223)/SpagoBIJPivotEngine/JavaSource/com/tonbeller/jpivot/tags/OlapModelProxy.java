...
public class OlapModelProxy extends OlapModelDecorator implements HttpSessionBindingListener {
  ...
  class MyState implements State {
    ...
    //*********** START CHANGES ****************
    //******************************************
    public void initialize(List dimAccList) throws Exception {
    	try {
            if (logger.isInfoEnabled())
              logger.info("initializing: " + model);
            model.initialize(dimAccList);
          } catch (OlapException e) {
            logger.error(null, e);
            throw e;
          }
   ...
   * and shown.
   */
  public void initializeAndShow(OlapModel model, List dimAccList) throws Exception {
    State s = new MyState(model);
    stateManager.initializeAndShow(s, dimAccList);
  }
  //******************************************
  //********** END CHANGES *******************
  //******************************************
   ...
   */
  public void initializeAndShow(String queryName, OlapModel model, List dimAccList) throws Exception {
