...
Rem                     - Use JONAS_BASE
Rem ---------------------------------------------------------------------------
Rem $Id: jonas.bat,v 1.37 2005/10/26 12:13:56 sauthieg Exp $
Rem ---------------------------------------------------------------------------

Rem Keep variables local to this script
setlocal ENABLEDELAYEDEXPANSION

set JAVA_OPTS= %JAVA_OPTS% -Xms256m -Xmx256m -XX:MaxPermSize=128m
...
%JAVA_CMD% wrapper.java.additional %JAVA_OPTS% %JONAS_OPTS% %TOMCAT_OPTS% %JETTY_OPTS% >>%WRAPPER_CONF%
...
:usage
echo "jonas start | stop | admin | version | check | -fg | -bg | -win | -n | -debug | -cfgsvc | -cnhost | -cnport"
