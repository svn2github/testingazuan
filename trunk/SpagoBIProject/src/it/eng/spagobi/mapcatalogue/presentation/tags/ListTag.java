				...
				if (parameters == null || parameters.size() == 0) {
					...
					for (int j = 0; j < parameters.size(); j++) {
						...
						if (parameterName.equalsIgnoreCase(PAR_NUM_MAPS)){		
							...
							if (new Integer(parameterValueObject.toString()) > 0)
								_flgMessageDel = true;
						}
					}
//					***************** fine prova ****************
					...
					String messageDel = "";
					if (_flgMessageDel) messageDel = msgBuilder.getMessage(_requestContainer, _messageDel, "component_mapcatalogue_messages");
